...
def features_types(data: pd.DataFrame) -> list:
    ...
    numeric_list = data.select_dtypes(
        include=["int64", "float64", "int32", "float32"]
    ).columns.tolist()
    cat_list = data.select_dtypes(
...
def data_props(
    ...
    data_props prend en argument \n
    une dataframe df et requiert le nom de la colonne de l'indicateur,
    ind_name ainsi qu'une liste d'indicateurs numériques.

    ENTREES:

    propsInd_data: Calcul la proportion de ind_name avec des
    valeurs renseignées par feature\n
    df_ind_name: Agrege le nombre de valeurs non nulle présente pour
    ind_name suivant chaque features de numeric_ind_list \n
    ind_sort_order: feature de référence pour le sorting de df_ind_name\n


    SORTIES:
    propsInd_data: Proportion de valeurs renseignées par
    groupe d'observations appartenant la feature indiquée par ind_name
    df_ind_name: Nombres de valeurs renseignées pour chaque
    groupe d'observation suivant les features présentes \n
    ...
    indicator_availability = (
        df[numeric_cols].notna().astype(int).groupby(df[ind_name]).mean()
    )
    ...
    propsInd_data = propsInd_data.sort_values(
        by="props_per_indicator(%)", ascending=False, ignore_index=True
    )
...
def top_score(
    ...
    """
    Fournit un classement général issue d'une pondération des paramètres
    issus de ind_pond et un classement absolu
    ...
    ind_ref: nom feature de référence pour distinguer les observations.
    en général la colonne modèle.
    ...
    for ind, sens in ind_weight.items():
        ...
        if sens[1] >= 0:
            rank_df.loc[:, ind] = data.loc[:, ind].rank(
                ascending=False, method="min", na_option="bottom"
            )
            # Calcul des rang ( rang 1 = dernier), NaN = rang 1
        else:
            rank_df.loc[:, ind] = data.loc[:, ind].rank(
                ascending=True, method="min", na_option="top"
            )

    # Moyenne pondérée: (\sum w_i*X_i)/(\sum w_i)
    # avec w_i les poids associés et X_i les indicateurs
    rank_df.loc[:, "score"] = sum(
        rank_df.loc[:, ind] * weight[0] for ind, weight in ind_weight.items()
    ) / sum(weight[0] for weight in ind_weight.values())
...
def iqr_outliers(datas: pd.DataFrame, iqr_coeff=1.5):
    ...
    datas : données\n
    iqr_coeff: coeff pour jauger longueur moustaches (ref: 1.5 de Tukey) \n
...
def zmad_outliers(datas: pd.DataFrame, zmad_coeff=3.5):
    ...
    # MAD: médiane de la val abs de la diff entre
    # la val de la feature et la médiane de la feature
    mad = (df_num - med).abs().median()

    for col in df_num.columns:
        z_mad = 0.6745 * (df_num[col] - med[col]) / mad[col]
