...
# Heatmap
def graphs_corr(data: pd.DataFrame, vmin: float, vmax: float,title_save="Heatmap", save = False)->plt.Figure:
    '''
    ...
    for i,meth in enumerate(corr_method):
        ...
        # abs_corr_matrix = abs(data.iloc[:-1,:-1].corr(numeric_only=True,method=meth))
